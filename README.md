...
Using data from the World Bank, the IDF Diabetes Atlas, and the CDC, a Tableau dashboard was built to gain an overall understanding of diabetes globally and in the United States. Global prevalence, health expenditure, and demographic information realting to diabetes were investigated in this dashboard. 
DB browser in conjuction with SQLite were used to create a database for machine learning. Considering that the outcome is binary (patient has diabetes or does not have diabetes), classification machine learning models were used. Then the balanced acuracy score was calcualted useing SMOTE for logisitic regression K-nearest neigbors support vector classifier and random forest classifier. The machine learning model with the highest random learning score was the random forrest classifier. We evaluated random forest machine learning model with a confusion matrix and accuracy result chart. 
